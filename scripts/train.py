...
def main():
    ...
    val_sets = {} if args.no_val else {
        _label(d): build_dataset(d) for d in cfg.get("val_datasets", [])}
    ...
    trainer = Trainer(
        model, train_set, cfg.train,
        device=args.device, val_sets=val_sets, metrics=metrics, out_dir=out_dir,
    )
    trainer.train()
