...
SwinIR là một transformer model, dùng cơ chế attention theo cửa sổ trượt (shifted
window) của Swin Transformer. Giống SRCNN, nó tạo ảnh HR trong một lần xử lý, nhưng
mạnh hơn nhiều nhờ attention. Khác với hai model trên, dự án dùng lại nguyên văn
kiến trúc chính thức của tác giả (`src/models/swinir_arch.py`), nhờ đó nạp trực
tiếp được trọng số đã train sẵn mà tác giả công bố. Vì vậy SwinIR không cần tự
train — chỉ tải trọng số về và chạy evaluate, làm mốc so sánh cho hai model kia.
File đăng ký: `src/models/swinir.py`, config: `configs/swinir_x4.yaml`.
...
# đánh giá SwinIR bằng trọng số train sẵn của tác giả (không cần train)
...
SRCNN và SR3 được cài đặt lại theo đúng ý tưởng của paper gốc, nhưng không phải bản
chính thức của tác giả. Vì vậy hai model này không nạp được các tham số đã train sẵn
mà tác giả gốc công bố. Riêng SwinIR dùng nguyên văn kiến trúc chính thức, nên nạp
được trọng số train sẵn của tác giả và chỉ dùng để đánh giá.
